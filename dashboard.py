...
# Try to generate processed data automatically
processed_file = Path("data/processed/master_performance.csv")

if not processed_file.exists():
    with st.spinner("Preparing data for the first time..."):
        try:
            subprocess.run(["python", "main.py", "--no-cards"], check=True)
            st.cache_data.clear()
        except Exception as e:
            st.error(f"Failed to generate processed data: {e}")
            st.stop()

master, module, daily = load_data()

if master is None:
    st.error("Processed data could not be loaded.")
    st.stop()
...
if view == "Admin Dashboard":
    ...
    # ── Bulk email sender ────────────────────────────────────────────────
    st.markdown('<div class="section-title">Send Grade Cards by Email</div>', unsafe_allow_html=True)

    with st.container():
        st.markdown(
            f"<p style='color:#94A3B8;font-size:13px;'>"
            f"This will email each student's grade card PDF to their registered address "
            f"({len(master)} students). Emails without a valid grade card PDF will be skipped."
            f"</p>", unsafe_allow_html=True
        )

        confirm_send = st.checkbox(
            "I understand this will send real emails to all students",
        ...
        if send_clicked:
            ...
            for _, r in master.iterrows():
                safe = r['email'].replace('@', '_at_').replace('.', '_')
                p = Path(f'output/grade_cards/{safe}.pdf')
                if p.exists():
                    grade_card_paths[r['email']] = str(p)

            progress_bar = st.progress(0, text="Starting email send...")
            status_box = st.empty()

            try:
                from src.emailer import send_all_grade_cards
                with st.spinner("Sending emails..."):
                    result = send_all_grade_cards(
                        master,
                        grade_card_paths,
                        dry_run=False
                    )
                ...
                status_box.success(
                    f"✅ Email send complete. Check `output/reports/email_log.csv` for full results."
                )
                if isinstance(result, dict):
                    ...
                    r1.metric("Sent", sent)
                    r2.metric("Failed", failed)
                    r3.metric("Skipped", skipped)
            except Exception as e:
                progress_bar.empty()
                status_box.error(f"❌ Email send failed: {e}")
                st.info(
                    "Make sure email credentials (SMTP settings / API key) are configured "
                    "in your .env file, and that src/emailer.py is reachable from this directory."
                )
    ...
    table_data = master[display_cols]

    if search_query:
        q = search_query.strip().lower()
        mask = (
            table_data['name'].str.lower().str.contains(q, na=False) |
            table_data['email'].str.lower().str.contains(q, na=False)
        )
    ...
    if search_query and table_data.empty:
        st.markdown(
            f"<p style='color:#64748B;font-size:13px;'>No students found matching '{search_query}'.</p>",
            unsafe_allow_html=True
        )
    else:
        st.dataframe(
            table_data.set_index('rank'),
            use_container_width=True,
            height=400,
            column_config={
...
# ══════════════════════════════════════════════════════════════════════════
    ...
    with dl_col:
        safe_name = email_query.replace('@', '_at_').replace('.', '_')
        pdf_path = Path(f'output/grade_cards/{safe_name}.pdf')
        if pdf_path.exists():
            with open(pdf_path, 'rb') as pdf_file:
                st.download_button(
                    label="⬇ Download\nGrade Card PDF",
                    data=pdf_file.read(),
                    file_name=f"{row['name'].strip().replace(' ', '_')}_grade_card.pdf",
                    mime="application/pdf",
                    use_container_width=True
                )
        else:
            st.markdown(
                "<p style='color:#64748B;font-size:11px;padding-top:32px;'>Grade card PDF not found.</p>",
                unsafe_allow_html=True
            )
    ...
    # Quiz-by-quiz trend
    if not daily.empty:
        student_daily = daily[daily['email'] == email_query].copy()
        if not student_daily.empty and 'date' in student_daily.columns:
            student_daily = student_daily.sort_values('date')
            st.markdown('<div class="section-title">Quiz-by-Quiz Trend</div>', unsafe_allow_html=True)

            score_col = 'percentage' if 'percentage' in student_daily.columns else None
            if score_col:
                fig_trend = go.Figure()
                fig_trend.add_trace(go.Scatter(
                    x=student_daily['date'],
                    y=student_daily[score_col],
                    mode='lines+markers',
                    line=dict(color='#3B9EFF', width=2),
                    marker=dict(
                        size=9,
                        color=[
                            '#22C55E' if v >= 70 else '#EAB308' if v >= 55 else '#EF4444'
                            for v in student_daily[score_col]
                        ]
                    ),
                    text=[f"{v:.1f}%" for v in student_daily[score_col]],
                    textposition='top center',
                    hovertemplate='%{x}<br>%{y:.1f}%<extra></extra>'
                ))
                avg_line = student_daily[score_col].mean()
                fig_trend.add_hline(
                    y=avg_line, line_dash='dot', line_color='#64748B',
                    annotation_text=f"Avg: {avg_line:.1f}%",
                    annotation_font_color='#64748B'
                )
                fig_trend.update_layout(
                    **PLOTLY_THEME,
                    xaxis=dict(title='Quiz Date', showgrid=False, tickfont=dict(color='#94A3B8')),
                    yaxis=dict(title='Score %', range=[0, 105], showgrid=True, gridcolor='#1E2D45'),
                    showlegend=False, height=260
                )
